...
public:
	...
	void run() {
		...
		int num = 0;
		int targetflag = 1;	//无人机2快到目标点时 改为到附近
		...
		while (flag) {
			...
			if ((target_ - pos).norm()>firmware->positionTolerance || v.norm()>firmware->velocityTolerance) {
				...
				//加入协同
				othpos=firmware->getOtherPosition();
				othpos << othpos.x() + biancha.x, othpos.y() + biancha.y;
				//两架无人机之间的向量
				auto dd = othpos - pos;
				offset<<dd.x() / dd.norm()*offsetSize, dd.y() / dd.norm()*offsetSize;
				Vector2d r = field_building->getFlux(pos+offset);
				...
				if (dd.norm() <= 10) {
					if (dist > 5) {
						offset << dd.x() / dd.norm() * 2, dd.y() / dd.norm() * 2;
						r = field_building->getFlux(pos + offset);
					}
				}
				...
				if (dd.norm() >= 10) {
					...
					if (dist < 10) {
						offset << dd.x() / dd.norm() * 1, dd.y() / dd.norm() * 1;
						r = field_building->getFlux(pos + offset);
					}
					if (dist < 5) {
						r = field_building->getFlux(pos);
					}
				}
				else {
					r = field_building->getFlux(pos);
				...
				if (dist < 20) {
					if (firmware->dronename == "Drone2"&& targetflag == 1)
					{
						target_ << target_.x(), target_.y()+6;
						field_building->stop();
						field_building->start(target_, speed_);
						targetflag = 0;
					}
				...
				if (dd.norm() <= 5) {
					if (dist > 5) {
						offset << dd.x() / dd.norm() * scale * 0.2, dd.y() / dd.norm() * scale * 0.2;
						r = r - offset;
					}
				}
				...
				if (dd.norm() >= 20) {
					if (dist > 5) {
						offset << dd.x() / dd.norm() * scale * 0.1, dd.y() / dd.norm() * scale * 0.1;
						r = r + offset;
					}
				}
		...
		*/
		...
		ofstream outfileps;
		if (firmware->dronename == "Drone1")
		{
			outfileps.open("data/record data/ps1.txt");
			outfileps.precision(20);
			for (auto& p : ps) {
				outfileps << p.x() << " " << p.y() << endl;
			}
		...
		{
			outfileps.open("data/record data/ps2.txt");
			outfileps.precision(20);
			for (auto& p : ps) {
				outfileps << p.x() << " " << p.y() << endl;
			}
		...
		if (firmware->dronename == "Drone1")
		{
			outfilevs.open("data/record data/vs1.txt");
			outfilevs.precision(20);
			for (auto& vel : vs) {
				outfilevs << vel.x() << " " << vel.y() << endl;
			}
		...
		{
			outfilevs.open("data/record data/vs2.txt");
			outfilevs.precision(20);
			for (auto& vel : vs) {
				outfilevs << vel.x() << " " << vel.y() << endl;
			}
		...
		if (firmware->dronename == "Drone1")
		{
			outfilers.open("data/record data/rs1.txt");
			outfilers.precision(20);
			for (auto& rv : rs) {
				outfilers << rv.x() << " " << rv.y() << endl;
			}
		...
		{
			outfilers.open("data/record data/rs2.txt");
			outfilers.precision(20);
			for (auto& rv : rs) {
				outfilers << rv.x() << " " << rv.y() << endl;
			}
