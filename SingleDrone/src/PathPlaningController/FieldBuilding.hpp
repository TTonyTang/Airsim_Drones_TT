...
public:
	...
	void start(const Vector2d& target, const double speed) {
		cout << "before field_building 3" << endl;
		flag = true;
		target_ = target;
		speed_ = speed;
		thd = thread(&FieldBuilding::run, this);
		cout << "field_building start ready 5" << endl;
	}
	...
	void run() {
		cout << "field_building runing 4" << endl;
		while (flag) {
			//cout << "building" << endl;
			period();
		}
...
private:
	...
	static void smoothing(const vector<Vector2d>& ps, vector<Vector2d>& viewPoints){
		...
		cout << "sizex：" << pxs.size() << endl;
		cout << "sizey：" << pys.size() << endl;
		...
		field_building::FieldMathUtil::meanSample(tempPoints, 100, viewPoints);	//取平均值 变成100个点
	}
