...
public:
	...
	/*
	static pair<int,int> halfPlain2Target(const vector<Vector2d>& nodes, const Vector2d& pos, const Vector2d& target) {
		auto direction2Terget = target - pos;
		const double theta = atan2(direction2Terget.y(), direction2Terget.x());
		//const double a = regularAngle(theta + M_PI / 2);
		const double b = regularAngle(theta - M_PI / 2);
		...
		int left = 0, right = 0;
		for (int i = 0; i < N; i++) {
			auto direction = nodes[i] - pos;
			if (almost_equal(regularAngle(atan2(direction.y(), direction.x())),b)) {
				right = i;
				break;
			}
		}
