...
public:
	...
	void start(const Vector2d& target, const double speed) {
		cout << "before controller start" << endl;
		flag = true;
		target_ = target;
		speed_ = speed;
		...
		cout << "controller start ready" << endl;
	}
	...
	void run() {
		...
		while (flag) {
			//if(num>0 && num%N==0){
				//const double xnoise = distribution(generator);
				//cout << xnoise << endl;
				//const double ynoise = distribution(generator);
				//cout << ynoise << endl;
				//firmware->setDisturbance((num%(2*N)==0?1:-1)*2.0);
			//}
...
private:
	...
	void period(const Vector2d& r, const Vector2d& pos, const double yaw_real, const Vector2d& v){
		...
		Vector4d ry;
		ry << r, v;
		Vector2d u = lqgController(dt,ry);
